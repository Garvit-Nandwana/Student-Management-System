...
class DatabaseConnection:
    def __init__(self, host="localhost", user="root", password="chamber713gringotts", database="school"):
        self.host = host
        self.user = user
    ...
    def connect(self):
        connection = msc.connect(host=self.host,
                                 user=self.user,
...
class SearchStudent(QDialog):
    ...
    def search(self):
        ...
        connection = DatabaseConnection().connect()
        cursor = connection.cursor()
        cursor.execute("SELECT * FROM students WHERE Name = %s", (student_data,))
        result = cursor.fetchall()
        rows = list(result)
        print(rows)
